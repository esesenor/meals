...
export const sequenceReview = catchAsync(async (req, res, next) => {
  ...
  if (!authorizationHeader) { //si el usuario no exuiste o no esta logueado
    return next(
...
export const updateReview = catchAsync(async (req, res, next) => {
  ...
  if (!restaurantUpdated) {
    return next(new AppError(`Restaurant with id: ${reviewData.restaurantId} not found`, 404)
    );
  }
  if (!reviewUpdated) {
    return next(new AppError(`Review with id: ${reviewData.userId} not found`, 404)
    );
  };
  const authorizationHeader = req.headers.authorization; //ver si esta el token autorizado
  const token = authorizationHeader.split(' ')[1]; //extraemos solo el token
  ...
  await ReviewService.update(userId, restaurantId, reviewId, reviewUpdated, reviewData);
